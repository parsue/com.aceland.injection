...
namespace AceLand.Injection
{
    ...
    public sealed class EntryPointRunner : MonoBehaviour
    {
        ...
        private readonly List<IAsyncEntryPoint> _async = new();
        ...
        /// <summary>Completes when every IAsyncEntryPoint of this scope finished (or faulted).</summary>
        ...
        public void Add(object ep)
        {
            ...
            if (ep is IAsyncEntryPoint a) _async.Add(a);
        ...
        private async Task RunAsyncStartablesAsync(CancellationToken ct)
        {
            ...
            foreach (var t in _async)
            {
                ...
                try
                {
                    var task = startable.RunAsync(ct);
                    if (task != null) await task;
                }
                catch (OperationCanceledException) { /* scope disposed */ }
                catch (Exception e)
                {
                    Debug.LogError($"[Injection] {startable.GetType().Name}.RunAsync failed:\n{e}");
                }
            }
        }
