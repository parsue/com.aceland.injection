...
#pragma warning restore UAC0005
            ...
            if (GlobalInstallerRegistry.Count > 0)
            {
                foreach (var e in GlobalInstallerRegistry.All())
                {
                    var i = Instantiate(e.InstallerType);
                    if (i != null) found.Add((e.Order, i));
                }
            }
            else
            {
                // Fallback (codegen disabled via ACELAND_INJECTION_NO_CODEGEN, or no
                // installers emitted): the original full-reflection scan.
