...
using System;
using AceLand.Injection.Editor.Licensing;
using AceLand.Licensing;
using UnityEditor;
using UnityEngine;

namespace AceLand.Injection.LicenseBridge
{
    /// <summary>
    ...
    /// </summary>
    [InitializeOnLoad]
    internal static class InjectionLicenseBridge
    {
        static InjectionLicenseBridge()
        {
            try
            {
                var descriptor = new LicenseProductDescriptor(
                    productId: "aceland_injection",
                    displayName: "AceLand Injection",
                    subscribeUrl: "https://subscription.parsue.io/l/injection",
                    trialDays: 7,
                    defaultMaxSeats: 1,
                    offlineGraceDays: 0,
                    storageFolder: null,
                    purchaseOnceUrl: "https://subscription.parsue.io/l/injection_permanent");
            ...
            catch (Exception e)
            {
                ...
                Debug.LogWarning($"[AceLand.Injection] License bridge init failed: {e.Message}");
            }
        }
    }
